 ...
 *
 */
public class MicroNominalInvestmentComputer extends AbstractMicroComputer implements
	...
	public Map<Long, Double> computeVariables(MacroSimulation sim) {
		...
		for (Agent i:pop.getAgents()){
			...
			if (!agent.isDead()){
			List<Item>loans=agent.getItemsStockMatrix(true, capGoodId);
			double investment=0;
			for (Item j:loans){
				CapitalGood good= (CapitalGood)j;
				if(good.getAge()<0)
					investment+=good.getValue();
				}
